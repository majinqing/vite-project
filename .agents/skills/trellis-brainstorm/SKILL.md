...
```bat
FOR /F "delims=" %%I IN ('
  .\.trellis\python.cmd .\.trellis\scripts\task.py create "brainstorm: <short goal>" --slug <auto>
') DO SET TASK_DIR=%%I
```
...
```bat
# Create child tasks
FOR /F "delims=" %%I IN ('.\.trellis\python.cmd .\.trellis\scripts\task.py create "Child task 1" --slug child1 --parent "%TASK_DIR%"') DO SET CHILD1=%%I
FOR /F "delims=" %%I IN ('.\.trellis\python.cmd .\.trellis\scripts\task.py create "Child task 2" --slug child2 --parent "%TASK_DIR%"') DO SET CHILD2=%%I

# Or link existing tasks
.\.trellis\python.cmd .\.trellis\scripts\task.py add-subtask "%TASK_DIR%" "%CHILD_DIR%"
```
